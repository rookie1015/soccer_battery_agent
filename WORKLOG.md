...
- Android 0.5.2（16）增加复盘期号的输入即保存与冷启动恢复，并在任九复盘期号右侧显示金额；旧历史记录缺少任九票对象时按保留场次票面补算。
- 全量测试 `249 passed, 11 subtests passed`，Android Debug 构建成功。API 36.1 模拟器安装确认 versionCode 16/versionName 0.5.2；将复盘期号改为 26119 后强制停止并重启，输入仍为 26119；生成任九复盘后标题右侧显示 `任九 ¥972`，未发现 App FATAL、Python traceback 或 ANR。
