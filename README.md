...
- Android：`0.5.14`（versionCode `28`）
- 更新日期：`2026-09-23`
- Python：`3.10+`
- 自动测试：`306` 项，另含 `656` 个子测试
...
- 国家队的 FotMob `squad` 用作长期球员池、位置结构、身价和纸面实力基线，但不单独裁定本期谁能出场。分析会从 FIFA 会员协会目录动态解析当期球队的足协官网，并检索临近比赛的退出消息：30 天内、球队归属明确的足协官方公告可直接确认；没有官方公告时，仍要求至少两家可信媒体相互印证。女足、青年队消息会被排除。相关标题、官网域名、退出人数和修正值写入报告快照，俱乐部阵容逻辑不受影响。
- 14 场预算票面使用正规复式计价：各场选择数量相乘得到注数，再按每注 2 元计算总成本。
- 最高购彩金额是成本上限；优化器在上限内联合选择单选、双选和全包，离散的复式乘积不保证恰好花满预算。
- “模型建议”保留预算处理前的判断范围；十四场预算优化会在金额上限内精确搜索单选、双选和全包的组合，最大化模型估算的 `P(14 场全中) + P(恰中 13 场)`。搜索使用每个成本状态下的帕累托前沿，不再只优化一等奖覆盖率，也不是逐场指定唯一失误的启发式。该概率以模型给出的单场概率及各场结果相互独立为前提，不代表实际中奖率或期望收益；任九只有一个奖级，仍独立最大化所选 9 场全部命中的概率。
...
| FIFA 会员协会目录 / 各足协官网 | 动态确认国家队所属足协官网、补充当期退出与征召公告 | 成年男足官方公告在时效内可单源确认；目录与官网长期缓存 |
...
| Google News / GDELT / DuckDuckGo | 新闻搜索与资料线索 | 通常仅展示；国家队名单消息只有足协官方单源确认或两家可信媒体交叉验证后才参与 |
...
3. 获取 FotMob / SofaScore 最近 20 场球队数据、xG 和阵容信息；国家队再从 FIFA 协会目录定位足协官网，补充当期成年男足名单公告。
...
10. 十四场保留预算处理前的模型建议，再在预算及已配置容差内精确最大化模型估算的“全中或恰错一场”概率；每场可重新分配单选、双选或全包。任九共享基础概率，但独立选择 9 个场次并最大化全中覆盖率，同时保持“原建议能容纳时不扩展、超预算时只压缩”的规则，两种玩法分别计价。
...
结构化经济数据保存在 `data/draw_economics/`，汇总写入 `reports/roi_backtest.md` 和同名 JSON。报告只评估理论推荐方案的历史回放表现。旧 Markdown 若没有任九独立票面，只计算可恢复的十四场票；无法确认独立线路成本的记录会标记为 `incomplete_cost`，不会进入累计 ROI。
