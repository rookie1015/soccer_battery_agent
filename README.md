...
- “模型建议”保留预算处理前的判断范围；十四场预算优化可在金额上限内重新分配单选、双选和全包，以最大化整票理论联合覆盖率。
...
10. 十四场保留预算处理前的模型建议，再在预算及已配置容差内最大化整票理论联合覆盖率；每场可重新分配单选、双选或全包。任九共享基础概率，但独立选择 9 个场次，并保持“原建议能容纳时不扩展、超预算时只压缩”的规则，两种玩法分别计价。
