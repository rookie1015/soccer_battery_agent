...
- 最高购彩金额直接换算成独立完整线路，每 2 元一条 14 场线路；500 元最多生成 250 条互不重复的线路。
- 模型原始建议中保留的每一个平局都会获得明确线路配额，并优先形成多平同线组合；不再只挑一场平局做对冲，也不再用矩形复式压缩后遗漏平局。
- 平局配额保证的是该平局在一定数量的线路中出现，不代表每条线路都含平，更不承诺整票命中；模型原始未选择的赛果仍无法由组合器补回。
...
7. 数学模型按 xG/进失球数据质量动态降权，再用伤停、阵容、近期表现、交锋平局率和盘口证据验证意外信号；关键资料大范围请求失败时直接报错，不再降级为简单分析。
