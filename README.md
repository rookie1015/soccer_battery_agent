...
球队匹配会优先使用已确认的 FotMob / SofaScore 球队 ID，再使用共享别名和完整赛程的主客队组合。由唯一赛程关系确认的新别名会写入缓存目录旁的 `team_identity.json`，下次分析和 App 升级后继续使用；低置信度候选只写入采集诊断，不会自动绑定。国家成年队、青年队和女足不会仅凭名称包含关系互相匹配。
